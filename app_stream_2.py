    ...
    async def receive_stt_text(self, deepgram_self, result, **kwargs):
        ...
        if result.is_final:
            ...
            if result.speech_final:
                ...
                print(f"Client {self.client_id} STT OUTPUT: {utterance}")
                self.is_finals = []
        ...
        try:
            while True:
                ...
                if text:           
                    ...
                    print(f"Client {self.client_id} TTT OUTPUT: {response}\tTIME TAKEN: {t2-t1:3f}sec {(t2 - t1)*1000:4f} ms")
    ...
    async def setup_connections(self):
        try:
            ...
            stt_options = LiveOptions(model="nova-3", language="en-US", smart_format=True, encoding="linear16", channels=1, 
                                    sample_rate=16000, interim_results=True, utterance_end_ms="1000", vad_events=True, endpointing=300,)
            
            tts_options = SpeakWSOptions(model="aura-2-thalia-en", encoding="linear16", sample_rate=16000,)
...
class Server:
    ...
    async def main(self):
        ...
        try:
            async with websockets.serve(self.handle_new_client, '0.0.0.0', 8000):
